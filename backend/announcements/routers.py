...
async def get_announcements(skip: int | None = None, limit: int | None = None, announcements_filter: AnnouncementFilter = FilterDepends(AnnouncementFilter), announcements_service: AnnouncementsService = Depends(get_announcements_service)):
    return await announcements_service.get_all(announcements_filter, skip, limit)
